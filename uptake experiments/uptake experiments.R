...
# ggplot(outliersremoveddat,aes(x = Subset, y = MFI)) +
#   geom_bar(stat='summary',aes(fill=Subset)) +
#   geom_jitter(width = .2, alpha = .6) +
#   stat_summary(geom="errorbar", fun.data="mean_sdl",fun.args=list(mult=1),
#                size=1, color="black", width=.4)+
#   scale_fill_viridis(discrete=T,begin=.8,end=0.1)+
#   # theme(legend.position = 'none')+
#   facet_wrap(~organexperimentf,nrow=2,labeller = as_labeller(experiment.labs))+
#   stat_pvalue_manual(stat.test, hide.ns = T,label = "p = {scales::pvalue(p.adj)}") +
#   scale_y_continuous(name="Normalized MFI")
...
metabtimepointsbal<-ggplot(outliersremoveddat,aes(x = Subset, y = MFI)) +
  ...
  stat_pvalue_manual(stat.test.bal, hide.ns = T,label = "p.adj.signif",size = 6,c(2.6,3,1.5,1.7,1.9,2.1,1.5,1.7,1.9,2.2,1.4,1.7,1.9,2.2,2.4)) +
  scale_y_continuous(name="Normalized MFI")

metabtimepointsbal
ggsave(file.path("./", paste0("metabtimepoints-bal-newnorm", Sys.Date(), ".png")), plot = metabtimepointsbal, width = 180,height = 100, units = "mm",dpi = 600)
